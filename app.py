...
def dashboard():
    ...
    return render_template(
        "dashboard.html", resources_by_category=resources_by_category
    )
...
def admin_dashboard():
    ...
    resource_count = conn.execute("SELECT COUNT(*) as count FROM resources").fetchone()[
        "count"
    ]
...
def admin_assign_resource():
    ...
    form.resource_id.choices = [
        (resource["id"], resource["name"]) for resource in resources
    ]
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            flash(
                f'Resource "{resource["name"]}" assigned to user "{user["username"]}" successfully.',
                "success",
            )
